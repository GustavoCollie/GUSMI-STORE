...
export const PurchaseOrderForm = ({ suppliers, products, onClose, onSubmit, loading, inline = false, initialData = null }) => {
    const [formData, setFormData] = useState({
        ...
        other_expenses_amount: initialData?.other_expenses_amount || 0,
        other_expenses_description: initialData?.other_expenses_description || '',
        expected_delivery_date: initialData?.expected_delivery_date ? initialData.expected_delivery_date.split('T')[0] : new Date().toISOString().split('T')[0]
    });
